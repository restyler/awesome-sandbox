...
* **[AgentTier](https://github.com/agenttier/agenttier):** An open-source, Kubernetes-native platform that provisions isolated, persistent sandboxes for both human developers and AI agents from the same `Sandbox` CRD. Each sandbox is a Pod with its own PVC (so stop/resume preserves files), a default-deny NetworkPolicy, a per-session ServiceAccount, and an optional gVisor RuntimeClass for kernel-level isolation. Sandboxes can run in `mode: code` (interactive browser terminal over WebSocket → SPDY exec) or `mode: agent` (driven via `POST /configure` to install code and `POST /invoke` to stream stdout/stderr/exit as Server-Sent Events). An optional warm-pod pool claims a pre-provisioned Pod + PVC in ~800 ms versus ~10 s cold start. Hierarchical (cluster → namespace) governance policies cap CPU/memory/storage, max sandboxes per user, allowed templates, approved image registries, and per-namespace agent-mode caps; violations return structured `policy_violation` errors. Reference images ship for Claude Code + Bedrock and a model-free LangGraph agent. Ships a single Helm chart, a Python SDK (`pip install agenttier`), a Go CLI, a React Web UI, and an authenticated in-cluster reverse proxy for forwarded ports. License: **Apache-2.0**. Self-hosted only — no managed SaaS offering.

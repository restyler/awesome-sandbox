...
- [4. In-Depth Platform Profiles](#4-in-depth-platform-profiles)
  ...
  - [4.2. Islo: Persistent Environments for Unattended AI Agents](#42-islo-persistent-environments-for-unattended-ai-agents)
...
| [**Islo** ↓](#42-islo-persistent-environments-for-unattended-ai-agents) | Firecracker (MicroVM) | 2025 | N/A | Proprietary | No | Yes | Persistent | Full | Long-Running & Stateful |
...
### **4.2. Islo: Persistent Environments for Unattended AI Agents**

* **Overview:** Islo is a managed cloud platform built specifically for AI agents that need to run unattended. Its key differentiator is **persistent environments** that survive disconnection - your entire development state (dependencies, environment variables, database state) is captured and follows you across devices and interfaces. The platform combines Firecracker microVM isolation with multi-interface access (terminal, Slack, mobile), approval workflows for sensitive agent actions, and high-fidelity API fakes for running agents at scale without hitting rate limits.
* **Website:** [islo.dev](https://islo.dev)
* **Documentation:** [docs.islo.dev](https://docs.islo.dev)  
* **Launch Date:** **2025** - Emerged to address the gap between ephemeral sandboxes and full development environments for autonomous agents.  
* **License:** **Proprietary** platform with usage-based pricing.  
* **Hosting:**  
  * **SaaS:** Yes. The primary offering is a fully managed cloud service with pay-per-use pricing based on CPU time, memory, and storage consumption.  
  * **Self-Hosted:** No. Islo is exclusively a managed service.  
* **Capabilities:**  
  * **Filesystem Access:** **Persistent** - Unlike ephemeral sandboxes that reset on disconnect, Islo captures your entire environment state. Install dependencies once, and they persist across sessions. This eliminates the "re-setup" problem that plagues other sandbox solutions.  
  * **Network Access:** **Full** - Sandboxes have unrestricted internet access. Services running inside can be exposed via secure URLs.  
  * **Workload Suitability:** Explicitly designed for **long-running, unattended** agent workloads. The persistent state model means agents can run overnight, be paused, and resumed without losing context.  
* **Unique Features:**
  * **Multi-Interface Access:** The same running environment is accessible from terminal, Slack, or mobile. Check on your agent or approve actions without opening a laptop.
  * **Approval Workflows:** When agents attempt sensitive operations, you receive notifications across all interfaces and can approve/deny in seconds. This enables truly unattended operation with human-in-the-loop safety.
  * **API Fakes at Scale:** For running hundreds of agents simultaneously, Islo provides high-fidelity, in-memory fakes for common APIs (GitHub, Jira, Slack, AWS). Agents think they're talking to real services but hit test doubles, avoiding rate limits and costs.
  * **Integrated Build Acceleration:** Leverages Incredibuild technology to dramatically speed up compilation and build times within sandboxes.
* **Pricing Model:** Usage-based with no minimum commitment. Billed per CPU-hour ($0.07), memory-hour ($0.04/GB), and storage-hour ($0.0007/GB). Example: an 8-hour overnight agent session costs approximately $0.88.
* **Target Use Cases:**
  - AI coding agents (Claude Code, Cursor) running unattended
  - Remote development with environment portability
  - Demo environments for sales engineering

...
* **[AgentsMesh](https://agentsmesh.ai):** An open-source AI Agent Workforce Platform that extends the CDE concept to autonomous coding agents. Unlike traditional CDEs built for human developers, AgentsMesh provisions remote AI workstations ("AgentPods") where AI agents (Claude Code, Codex CLI, Gemini CLI, Aider, OpenCode) execute code.
  * **Sandboxing Approach:** Each AgentPod combines **PTY-based process isolation** ([`runner/internal/runner/pod_io_pty.go`](https://github.com/AgentsMesh/AgentsMesh/tree/main/runner/internal/runner)) with **git worktree isolation** ([`runner/internal/workspace/`](https://github.com/AgentsMesh/AgentsMesh/tree/main/runner/internal/workspace)) — agents get an isolated filesystem branch per task, eliminating cross-task contamination without full container overhead. Self-hosted runners can deploy inside customer infrastructure so source code never crosses the network perimeter. The control plane (Go backend, gRPC+mTLS) is architecturally separated from the data plane (WebSocket Relay), mirroring Gitpod's zero-trust split but tuned for agent orchestration rather than human IDE sessions. License: BSL-1.1 (transitions to GPL-2.0-or-later on 2030-02-28).

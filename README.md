...
* **[ClawSec](https://clawsec.cc):** A security audit platform for AI agent skills and MCP servers that uses Firecracker microVM sandboxes to safely execute and analyze untrusted code. ClawSec performs a 5-tier audit pipeline — static analysis, pattern matching, LLM semantic analysis, Firecracker sandbox execution, and LLM review — to detect malicious patterns, data exfiltration, and prompt injection. The sandbox execution tier runs each skill inside an isolated Firecracker microVM to observe actual runtime behavior without risking the host environment. Available as a SaaS platform with a searchable audit database of 30,000+ analyzed skills. Website: [clawsec.cc](https://clawsec.cc).

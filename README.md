...
* **[aicontainer](https://github.com/stefanoginella/aicontainer):** An MIT-licensed, self-hosted CLI that generates a sandboxed [Dev Container](https://containers.dev/) for running AI coding agents such as Claude Code and Codex in their auto-approve modes (`--dangerously-skip-permissions` / sandbox-off).
  * **Sandboxing Approach:** Isolation is **container-based** (Docker/OCI). Only the project directory is writable; the host home, SSH agent, and credentials are not forwarded. The Docker socket is exposed through a filtered [socket-proxy](https://github.com/Tecnativa/docker-socket-proxy) rather than mounted directly, and an opt-in iptables allowlist can restrict outbound network access. It targets the **AI/Agent-specific, self-hosted** quadrant, trading a microVM's stronger kernel isolation for the zero-setup experience of a local Docker container, and is explicit that the socket-proxy is a footgun-reducer rather than a hard host-isolation boundary.
